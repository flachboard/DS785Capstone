...
install.load::install_load("arrow", "bayestestR",
                           ...
                           "heatmaply", "hrbrthemes", "janitor", "latticeExtra", "lubridate", "maptools", "Metrics", "NbClust",
                           "OpenML", "openxlsx", "outliers",
                           "plotly", "poliscidata", "prophet", "psych", "randomForest", "ranger", "RcppRoll",
...
final_df %>%
  dplyr::filter(year(date) == 2021) %>%
  dplyr::select(cases_7dayavg_per100k, deaths_7dayavg_per100k, all_of(daily_vars)) %>%
  corrr::correlate() %>%
  ...
  ggplot(aes(term, cases_7dayavg_per100k)) +
    ...
    #labs(title  ="Daily Variables: Correlation with Cases Per 100k (7 Day Avg)") +
...
final_df %>%
  ...
  ggplot(aes(term, deaths_7dayavg_per100k)) +
    ...
    #ggtitle("Daily Variables: Correlation with Deaths Per 100k (7 Day Avg)") +
    scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
    theme_ipsum(base_family = "Futura Medium") +
    theme(axis.title.y = element_blank(), axis.title.x = element_blank(), panel.grid.minor = element_blank())
...
final_df %>%
  ...
  ggplot(aes(term, monthly_cases_per100k)) +
    geom_col(fill = plotly_colors[1]) + coord_flip() +
    #ggtitle("Monthly Variables: Correlation with Cases Per 100k") +
    scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
    theme_ipsum(base_family = "Futura Medium") +
    theme(axis.title.y = element_blank(), axis.title.x = element_blank(), panel.grid.minor = element_blank()) +
...
final_df %>%
  ...
  ggplot(aes(term, monthly_deaths_per100k)) +
    geom_col(fill = plotly_colors[2]) + coord_flip() +
    #ggtitle("Monthly Variables: Correlation with Deaths Per 100k") +
    scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
    theme_ipsum(base_family = "Futura Medium") +
    theme(axis.title.y = element_blank(), axis.title.x = element_blank(), panel.grid.minor = element_blank())
...
ggplot(aes(term, annual_cases_per100k)) +
  geom_col(fill = plotly_colors[1]) + coord_flip() +
  #ggtitle("Annual Variables: Correlation with Cases Per 100k") +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
  theme_ipsum(base_family = "Futura Medium") +
  theme(axis.title.y = element_blank(), axis.title.x = element_blank(), panel.grid.minor = element_blank())
...
ggplot(aes(term, annual_deaths_per100k)) +
  geom_col(fill = plotly_colors[2]) + coord_flip() +
  #ggtitle("Annual Variables: Correlation with Deaths Per 100k") +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
  theme_ipsum(base_family = "Futura Medium") +
  theme(axis.title.y = element_blank(), axis.title.x = element_blank(), panel.grid.minor = element_blank())
...
ggplotly(
  final_df %>%
    dplyr::filter(year(date) == 2021) %>%
    dplyr::select(cases_7dayavg_per100k, deaths_7dayavg_per100k, all_of(vax_vars)) %>%
    corrr::correlate() %>%
    corrr::focus(cases_7dayavg_per100k) %>%
    dplyr::mutate(term = reorder(term, cases_7dayavg_per100k)) %>%
    ggplot(aes(term, cases_7dayavg_per100k)) +
      ...
      theme_minimal() +
      theme(axis.title.y = element_blank(), axis.title.x = element_blank(), panel.grid.minor = element_blank()) +
      ggtitle("Cases Per 100k (7 Day Avg) & Vax/Deaths Correlation")
)
...
ggplotly(
  final_df %>%
    ...
    ggplot(aes(term, deaths_7dayavg_per100k)) +
      ...
      theme_minimal() +
      theme(axis.title.y = element_blank(), axis.title.x = element_blank(), panel.grid.minor = element_blank()) +
      ggtitle("Deaths Per 100k (7 Day Avg) & Vax/Cases Correlation")
)
...
# 
# #Final Model
# fit_xgb_cases_watch <- xgboost(params, data = dtrain, nrounds = 19, verbose = 0)
# 
# #Save Model
...
#Predict
train_pred <- predict(fit_xgb_cases_watch, train_x)
test_pred <- predict(fit_xgb_cases_watch, test_x)
...
# 
# #Final Model
# fit_xgb_deaths_watch <- xgboost(params, data = dtrain_deaths, nrounds = 16, verbose = 0)
# 
# #Save Model
...
adi_sdoh_economic <- final_df %>%
  dplyr::filter(year(date) == 2021) %>%
  select(fips, daily_deaths, contains("pct_m"), DEMOCRAT, OTHER, REPUBLICAN, GREEN, LIBERTARIAN, contains("income"), population, density) %>%
  group_by(fips) %>%
  summarise(total_deaths = sum(daily_deaths),
            ...
            conpct_m = mean(conpct_m),
            modpct_m = mean(modpct_m),
            libpct_m = mean(libpct_m),
            reppct_m = mean(reppct_m),
            indpct_m = mean(indpct_m),
            dempct_m = mean(dempct_m),
            Income = mean(Income),
            IncomePerCap = mean(IncomePerCap),
...
#Cluster
adi_sdoh_health_care_cluster <- kmeans(nor_hc, centers = 4)
...
#   ungroup() %>%
#   mutate(vax_group = cut_number(Series_Complete_Pop_Pct, 8)) %>%
...
#     geom_line(data=vax_group %>% dplyr::select(-vax_group), aes(group=vax_group_), color="lightgrey", size=0.5, alpha=0.5) +
#     geom_line(aes(color=vax_group), color = my_colors[2], size=1.25) +
#     theme_minimal() +
#     theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.title.x = element_blank()) +
#     facet_wrap(~vax_group, ncol = 2) +
#     ylab("7 Day Rolling Mean - Daily Cases Per 100k")
```
